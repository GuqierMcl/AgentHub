...
import type { ConversationAgentProfile, WorkbenchTimelineQuestionItem } from "@/features/workbench/types"
import type { InstructQuestionAnswerBody } from "../types"
...
type InstructQuestionAnswerComposerProps = {
...
export function InstructQuestionAnswerComposer({
  agentProfiles,
  onAnswerQuestion,
  onSkipRun,
  requests,
}: InstructQuestionAnswerComposerProps) {
  ...
  const currentDrafts = useMemo(
    () => activeRequest ? draftsByRequest[activeRequest.requestId] ?? {} : {},
  ...
  const missingRequired = activeRequest
    ? activeRequest.questions.filter((question) =>
        question.required && !isQuestionAnswered(currentDrafts[question.id])
      )
    : []
  ...
  const submit = useCallback(async () => {
    ...
    const answers: InstructQuestionAnswerBody["answers"] = activeRequest.questions.flatMap<InstructQuestionAnswerBody["answers"][number]>((question) => {
    ...
    setError(null)
    try {
      await onAnswerQuestion(activeRequest.runId, activeRequest.requestId, { answers })
      setSubmittedRequestId(activeRequest.requestId)
    } catch (err) {
      setError(err instanceof Error ? err.message : "问题回答失败")
      setSubmittedRequestId(null)
    } finally {
      setSubmittingRequestId(null)
    }
  }, [activeRequest, canSubmit, currentDrafts, onAnswerQuestion])

  const skipRun = useCallback(async () => {
    if (!activeRequest || skippingRequestId !== null || submittingRequestId !== null) {
      return
    }
  ...
  const activeDraft = activeQuestionIdValue ? currentDrafts[activeQuestionIdValue] : undefined
  const canGoNext =
    Boolean(nextQuestion) &&
    !busy &&
    Boolean(!activeRequest.questions[activeQuestionIndex]?.required || isQuestionAnswered(activeDraft))

  return (
    <div className="shrink-0 bg-transparent p-3 sm:p-4">
      <Card className="mx-auto max-h-[min(52vh,34rem)] max-w-3xl border bg-card shadow-sm" size="sm">
            ...
            >
              ...
              >
                {pendingRequests.map((request, index) => (
                  <TabsTrigger className="min-w-24 flex-none rounded-none px-3 py-2" key={request.requestId} value={request.requestId}>
                    请求 {index + 1}
                  </TabsTrigger>
                ))}
              </TabsList>
        ...
        <CardContent className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto">
          <Tabs className="w-full flex-col gap-4" onValueChange={setSelectedQuestionId} value={activeQuestionIdValue}>
            {activeRequest.questions.length > 1 ? (
              <TabsList className="grid h-auto w-full grid-flow-col auto-cols-[minmax(11rem,1fr)] overflow-x-auto rounded-full p-1">
                {activeRequest.questions.map((question, index) => {
                  const answered = isQuestionAnswered(currentDrafts[question.id])
                  return (
                    <TabsTrigger
                      className={cn(
            ...
            {activeRequest.questions.map((question) => {
              ...
              const radioValue = draft.custom
                ...
                : draft.optionId
                  ...
                  : ""
              return (
                <TabsContent className="mt-2" key={question.id} value={question.id}>
                      ...
                      >
                        {question.options.map((option) => {
                          ...
                          return (
                            ...
                            >
                              <RadioGroupItem id={inputId} value={`option:${option.id}`} />
                              <FieldContent className="min-w-0">
                                <FieldLabel className="w-auto min-w-0 whitespace-normal break-words" htmlFor={inputId}>
                                  {option.label}
                                </FieldLabel>
                                {option.description ? (
                                ...
                                {getVisibleOptionValue(option) ? (
                                  <div className="mt-2 overflow-x-auto rounded-lg border bg-muted/40 p-3 font-mono text-xs leading-6 whitespace-pre-wrap break-words text-foreground/90">
                                    {getVisibleOptionValue(option)}
                                  </div>
                                ) : null}
                              </FieldContent>
                        ...
                        {question.allowCustom ? (
                          <Field
                            ...
                            orientation="horizontal"
                          >
                            <RadioGroupItem id={`${activeRequest.requestId}-${question.id}-custom`} value="custom" />
                            <FieldContent className="min-w-0 flex-1">
                              {isCustomSelected ? (
                                ...
                                <FieldLabel className="w-auto min-w-0 whitespace-normal break-words" htmlFor={`${activeRequest.requestId}-${question.id}-custom`}>
                                  自定义答案
                                </FieldLabel>
                              )}
        ...
        <CardFooter className="justify-between gap-3">
          <div className="min-w-0 text-muted-foreground text-sm">
            <span className="block truncate">
              {missingRequired.length > 0 ? `还有 ${missingRequired.length} 个必填问题` : "可以提交"}
            </span>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <Button disabled={busy} onClick={() => void skipRun()} type="button" variant="outline">
              {skipping ? (
...
function getVisibleOptionValue(option: {
  label?: string
  description?: string
  value?: string
}): string | null {
  ...
  if (value === option.label?.trim() || value === option.description?.trim()) {
    return null
  }
