...
5. `docs/contracts/API_CONTRACTS.md` when the change affects public Runtime APIs, event payloads, errors, or authoring options
...
- Ensure execution enters through `RuntimeToolRegistry.executeTool()` so `tool.started`, `tool.completed`, and `tool.failed` are emitted consistently.
...
The Tool Catalog is the single code source for tool risk, permissions, approval policy, and authoring metadata. Do not rebuild tool lists in routers, CRUD validation, or UI-support code.
