...
- 重新生成助手回复使用追加候选分支模型：HubServer 复制源 Run 的 trigger user message 创建新 Run，新 assistant `Message.regeneratedFromId` 指向被重新生成的源 assistant message；新 trigger user message 的 `metadataJson.regenerate` 保存 source assistant、source run、source trigger 和 excerpt 快照，作为内部/外部 replay 的稳定语义来源。旧 assistant、旧 RunEvent 和旧 Artifact 不被覆盖或删除。
...
- Diff Artifact Detail 通过 `GET /api/conversations/:conversationId/artifacts/:artifactId` 读取。HubServer 从当前 `ArtifactVersion.diffJson` 派生 `changedFiles`、`baselineDirty`、`runOnlyReliable`、`limitations` 等详情字段，并从 `ArtifactVersion.content` 或 `diffJson.patch.text` 提供 bounded patch 文本；若 artifact 已有关联 `WorkspaceChangeSet`，响应额外返回 `diff.changeSet` 和 `changedFiles[].attribution`。旧 Diff Artifact 没有关联 ChangeSet 时继续返回原有详情，Web 显示“归因正在同步”。可靠的原始 Diff Artifact 可通过 HubServer revert 产品 API 执行完整 Run 级撤销；撤销成功后创建一条系统 assistant `Message`，挂载新的 `Artifact(type="diff")`，`Artifact.metadataJson.source = "workspace.revert"`，`ArtifactVersion.source = "diff_apply"`，`ArtifactVersion.diffJson.operation.patchDirection = "reverse-applied"`，并创建新的 `WorkspaceChangeSet` / files 记录撤销操作。撤销记录展示的是被撤销的原始 patch，不表示二次可撤销变更。
