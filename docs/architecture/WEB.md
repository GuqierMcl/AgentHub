...
- 切换到已有会话且 message snapshot 正在加载时，聊天区保留当前面板结构，并在聊天框上覆盖半透明背景模糊加载层，避免先闪现“无消息”或空白消息区。
...
- Zustand 管理客户端运行态和 UI overlay：`activeConversationId`、per-conversation draft、由 `timelineRuns` 产品 event replay hydrate 出来的 timeline items、当前 HubServer active run id、Run 状态、SSE 连接状态、已收到 Runtime event ids、轻量 event log、产物工作台 tab、折叠状态和自动聚焦请求。会话列表可用该本地状态覆盖已打开会话的最近消息预览和运行状态；未打开会话不显示运行状态。聊天主体父级布局只订阅 deployment、run id/status 和连接状态等低频外壳字段；timeline 由消息流和实际需要上下文统计的右侧 tab 自己订阅，draft 由 composer 层订阅，避免输入每个字符或收到每个 streaming delta 时重渲染父级三栏布局、消息流以外的 header/composer 和产物工作台外壳。`ResizablePanel` 等 DOM imperative ref 不进入 store，布局组件只消费 store 意图并执行展开/折叠。
...
- 发送消息或重新生成成功后，Web 将 HubServer 返回的 messages snapshot 写入 TanStack Query，并用该 query 的 `dataUpdatedAt` 标记本轮 snapshot 已处理；同一份结果只能 hydrate 一次，避免成功回调和 query effect 连续重放最近窗口造成发送后卡顿。
...
- assistant 消息在 `status === "streaming"` 时，无论当前是否已经有正文内容，消息底部都应保留稳定的“正在生成...” 指示，避免切换会话后只剩正文却看不出仍在流式输出。为保护流式输出热路径，streaming assistant 正文使用轻量纯文本渲染，不在每个 delta 期间反复执行完整 Markdown / Streamdown 解析；消息完成、历史 replay 或非 streaming 消息再使用标准 Markdown 渲染。
