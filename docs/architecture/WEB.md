...
- 聊天输入框下方的 MCP 状态栏只调用 HubServer `/api/conversations/:conversationId/mcp/status`。该状态来自 Runtime 去重后的有效 workspace MCP server 列表：同一 MCP 同时安装在 Codex、Claude Code、OpenCode 等来源时只显示一个状态 pill；具体采用来源、可发现来源集合和重复候选数由 Runtime status response 提供。Web 不自行读取配置文件、不传 `rootPath`，也不直接连接 MCP server。
