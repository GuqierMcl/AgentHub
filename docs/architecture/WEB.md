...
- Zustand 管理客户端运行态和 UI overlay：`activeConversationId`、per-conversation draft、由 `timelineRuns` 产品 event replay hydrate 出来的 timeline items、当前 HubServer active run id、Run 状态、SSE 连接状态、已收到 Runtime event ids、轻量 event log、产物工作台 tab、折叠状态和自动聚焦请求。会话列表可用该本地状态覆盖已打开会话的最近消息预览和运行状态；未打开会话不显示运行状态。聊天主体父级布局只订阅 timeline、deployment、run id/status 和连接状态等渲染字段；draft 由 composer 层订阅，避免输入每个字符时重渲染消息流和产物工作台。`ResizablePanel` 等 DOM imperative ref 不进入 store，布局组件只消费 store 意图并执行展开/折叠。
