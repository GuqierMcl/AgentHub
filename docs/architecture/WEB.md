...
- `deployment.*` 事件由 deployment preview reducer 消费，不作为普通工具卡片主事实。右侧部署预览标签页应从 replay/live 事件恢复服务器展示信息、连接状态、进度、步骤、只读 SSH 日志、release note、部署 URL、health check 和终态；前端 mock deploy data 只能作为开发占位，不得进入真实会话渲染路径。部署预览内容必须像 Diff Viewer 一样随右侧产物工作台父容器宽度弹性变化，卡片、命令、日志、release note 和底部按钮都不得撑破父容器。
