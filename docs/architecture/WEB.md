...
- 聊天输入框在任意会话类型输入 `@` 都打开 mention 候选；群聊候选显示可直接 @ 的成员主智能体并排除 `orchestrator`，候选行头像必须复用共享智能体头像 resolver 与用户头像覆盖配置，非群聊当前只显示空状态。当前阶段只允许结构化选择一个 @ 智能体；纯手打 `@Agent` 文本不改变路由。
...
- 聊天模块的会话列表、会话详情、新建、重命名、置顶和归档已经接入 HubServer conversation API；列表卡片使用 `lastMessageContent` 显示最近一条文本消息，HubServer 返回前最多截取 50 个字符，前端保持单行 `truncate` 展示。会话列表由 HubServer 排序：置顶会话在最前，未置顶会话按 `lastMessageAt ?? createdAt desc` 排序，新建空会话会默认排在其他未置顶会话之前。消息发送与恢复当前走 HubServer 产品级 messages/runs API：Web 调用 `POST /api/conversations/:conversationId/messages/send` 或 regenerate API 获取轻量 fast ack，再通过 `GET /api/conversations/:conversationId/messages` 获取最近窗口的恢复快照。恢复快照默认只取最近 10 条消息和最近 10 个 run。刷新页面后，聊天主 UI 通过重放产品 event envelopes 恢复，并用 `messages` 中的持久化 chat user/assistant 消息做兜底；完整 raw Runtime event 留在 HubServer `RunEvent.payloadJson`。用户向上滚到聊天顶部时，Web 改为调用 `GET /api/conversations/:conversationId/messages/history?cursor=&limit=` 自动加载更早一页；该接口只服务历史上翻，不参与发送热路径，且首屏进入会话时不会因为顶部 sentinel 已在视口中就立刻补拉 history。
...
- 重新生成 V1 只作用于已完成 assistant chat 消息。Web 在 assistant 消息 action 行提供“重新生成”图标按钮，点击后调用 HubServer regenerate API，并进入与普通发送相同的 submitted/running/SSE hydrate 流程；hydrate/merge 持久化消息时保留 `metadataJson.regenerate` 和 `regeneratedFromId`。复制出的 user trigger 仍作为 Run 触发事实持久化，但当原始 user 消息在当前 timeline 窗口内时不单独渲染为新的用户气泡，而是折叠到原始 user 消息底部显示“已请求重新生成 N 次”；如果原始 user 消息不在窗口内，trigger 保留紧凑“重新生成请求”标记作为分页兜底。如果源 assistant 在当前 timeline 窗口内，新替代回复会折叠进源回复的 `MessageBranch` 版本分页，并默认显示最新候选；原始回复和每个 regenerated 候选都在消息操作/元信息栏显示“已重新生成”，regenerated 候选气泡顶部仍显示“重新生成回复”。V1 不做设为首选答案、自动隐藏旧回复或线程视图。
...
- Web 通过 HubServer 的产品级 `/api/conversations/:conversationId/messages*` 与 `/api/runs/:runId/*` API 发送消息、续订事件流和取消 Run；浏览器仍不得直接调用 `agent-runtime`。`/api/runtime/runs*` 仅保留为调试代理，不是聊天主路径。设置页“输出设置”读写 HubServer `/api/settings/diagnostics`，真实聊天 Run 创建时由 HubServer 读取当前输出设置并注入 Runtime `RunInput.diagnostics`；浏览器发送消息请求不携带这些 Runtime 内部字段。
...
- 设置页“AI能力 -> 插件配置”展示 Runtime Capability Discovery 返回的 Skill / MCP metadata。AgentHub 产品侧只暴露“全局”和“工作区/项目级”两个范围；Web 只调用 HubServer `/api/runtime/capabilities*`，不直接访问 Runtime。工作区视图未选择会话时，HubServer 遍历 active conversations，按 canonical `rootPath` 去重并返回 `workspaces[]` 分组；选择某个会话时只返回该会话绑定的 workspace 分组。Web 不自行解析或配对 rootPath，只按 HubServer 返回的 workspace 分组纵向渲染可折叠大卡片，并可用 rootPath 作为卡片路径提示。浏览器请求体不传 `rootPath`，也不让 Runtime 猜路径。Workspace Skill trust 操作只调用 HubServer `/api/runtime/workspace-skill-trust*`，请求体只包含 `conversationId`、Skill ref 和信任决策；自动发现的 workspace Skill 默认 trusted，UI 中的操作语义是显式撤销或恢复允许。
- 聊天输入框下方的 MCP 状态栏只调用 HubServer `/api/conversations/:conversationId/mcp/status`。该状态来自 Runtime 去重后的有效 workspace MCP server 列表：同一 MCP 同时安装在 Codex、Claude Code、OpenCode 等来源时只显示一个状态 pill；具体采用来源、可发现来源集合和重复候选数由 Runtime status response 提供。Web 不自行读取配置文件、不传 `rootPath`，也不直接连接 MCP server。同一状态栏还展示当前会话 live deployment snapshot 中的常驻 SSH 连接状态；仅 `connecting` / `connected` 显示为 SSH pill，例如“已连接：公司服务器”，`stale` / `disconnected` / `failed` 不显示，避免历史 replay 误导用户。
...
- 当前发送路径不做浏览器侧 user message 乐观插入。提交后 Web 先清空 draft 并把本地 run 状态置为 `submitted`；收到 `messages/send` 或 regenerate 的 fast ack 后，只轻量合并当前新 run 的 trigger user message、重置该 run 的本地 event 去重状态并尽早建立产品 SSE 连接。发送成功回调不再把完整 `messages` 快照写入 TanStack Query cache；完整恢复快照只由 `GET /api/conversations/:conversationId/messages` 提供并在首次进入、切回会话、刷新页面或显式 refetch 时 hydrate。
- 聊天历史分页与恢复路径解耦：`GET /api/conversations/:conversationId/messages` 仍返回完整恢复快照，并额外提供 `history.hasOlder / history.nextCursor` 作为首个历史游标；`GET /api/conversations/:conversationId/messages/history` 只返回 `messages + timelineRuns + page`，前端当前按每页 5 个历史根节点请求。历史页按“历史根节点”分页，run 根节点会返回完整 `timelineRuns.events` 与该 run 相关的 persisted chat messages；standalone persisted chat message 作为独立根节点返回。历史页 prepend 时不得重置当前 `activeRuntimeRunId`、`runStatus`、`connectionStatus` 或 live SSE 去重状态；首版已上翻出的旧页只保留在当前会话内存态，刷新或切换会话后允许丢失并回到最近恢复窗口。
...
- HubServer 全局事件流 `GET /api/events` 独立于产品 Run SSE。全局事件作为 query invalidation、已打开会话运行态和系统服务状态更新信号；它不进入 Runtime timeline projection，也不影响产品 event replay 顺序。`service.status.changed` 只更新共享 service status store，不写入 conversation timeline、不落库、不参与 replay。聊天输入框下方渲染当前会话中外部智能体（OpenCode、Claude Code、Codex 等）的持续状态栏；该状态栏直接读取 service status store，显示“运行中 / 启动中 / 待命 / 错误 / 未接入”等当前快照，空会话或没有外部智能体成员时不显示。
...
- Permission 卡片必须渲染 Runtime approval payload 中的安全详情，而不能只显示 `toolName` 或目标。部署远程命令展示服务器、用户、命令、cwd 和原因；`bash` 展示命令、cwd、shell 和匹配规则；`web_fetch` 展示方法、脱敏 URL 和 host；workspace 审批展示逻辑路径、访问模式、目标类型和原因。长命令、URL 和路径必须在卡片内部换行或横向滚动，不能撑破聊天消息宽度。
- `deployment.*` 事件由 deployment preview reducer 消费，不作为普通工具卡片主事实。右侧部署预览标签页应从 replay/live 事件恢复服务器展示信息、连接状态、进度、步骤、只读 SSH 日志、release note、部署 URL、health check 和终态；前端 mock deploy data 只能作为开发占位，不得进入真实会话渲染路径。部署预览内容必须像 Diff Viewer 一样随右侧产物工作台父容器宽度弹性变化，右侧工作台容器、活动 tab wrapper、标签标题、内容标题、卡片、命令、日志、release note 和底部按钮都不得撑破父容器。
