...
- 左侧一级导航在模块导航下方、用户栏上方展示系统服务状态面板。Web 首次加载时读取一次 `GET /api/system/services/status`，后续通过 HubServer 全局事件 `service.status.changed` 更新状态，不做浏览器侧轮询，也不直接访问 Agent Runtime 或 OpenCode server。展开导航时显示 AgentRuntime、OpenCode、Codex、Claude Code 的紧凑中文状态；折叠导航时显示聚合状态点与 tooltip。OpenCode/Claude Code `idle` 显示为“待命”；Claude Code 有非终态 direct/delegated Run 时由 Runtime 上报 `running`，显示为“运行中”；Codex/Claude Code 未接入时显示“未接入”。
...
- HubServer 全局事件流 `GET /api/events` 独立于产品 Run SSE。全局事件作为 query invalidation、已打开会话运行态和系统服务状态更新信号；它不进入 Runtime timeline projection，也不影响产品 event replay 顺序。外部智能体 service status 从 `error/not_integrated` 进入 `starting/idle/running` 时，Web 可在当前 active conversation 追加一条本地 `service_status_notice` 分隔提示，例如 `Claude Code · 已启动`；当用户进入包含外部智能体的会话且对应服务已经可用时，Web 也会为该会话补一条同类基线提示，避免服务在打开会话前已启动导致聊天流无感知。`idle <-> running` 只更新服务状态面板，不追加聊天提示。
