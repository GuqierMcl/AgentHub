...
- Web 聊天主 UI 使用产品 event replay 恢复，并与 live SSE 共用同一套 projection reducer；完整 raw Runtime event 保存在 `RunEvent.payloadJson`。
...
3. 聊天 UI 恢复时先插入该 run 的 trigger user message，再按 `RunEvent.sequence asc` 重放产品 event envelopes；产品 envelope 中 `event.runId` 是 Hub 本地 Run id，`event.runtimeRunId` 是 Runtime run id。
...
- `GET /api/conversations/:conversationId/messages` 返回 `timelineRuns`，每个 run 包含 trigger user message 和按 `sequence asc` 排序的产品 event envelopes。
...
- 重开同一对话时，live 渲染和产品 replay 渲染必须得到同一顺序。
