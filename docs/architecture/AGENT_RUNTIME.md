...
Runtime 还支持独立的系统智能体层，用于自动执行不属于用户可见主智能体和隐藏子智能体的维护任务。首版系统智能体为 `title`：仅在会话第一次对话对应的 Run 中触发，继承当前 Run 入口智能体的模型快照生成短标题。若标题结果在 `run.completed` 前完成，Runtime 会在同一条 Run SSE 中输出 `system_agent.completed`；若没有赶上则取消标题任务并跳过，不延迟主 Run 完成。Runtime 不直接更新会话标题；HubServer 消费该事件并在标题未被用户手动修改时落库。
