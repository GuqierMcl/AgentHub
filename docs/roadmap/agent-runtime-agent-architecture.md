...
第一轮实现进行中：阶段 1 作为本轮主目标，阶段 3 中的只读 Agents 查询 API 作为附带完成项。Run API、事件流、Executor、Orchestrator V1 和外部 Adapter 实接仍保留到后续阶段。
...
- 实现内部智能体统一执行接口。
- 实现 `MockExecutor` 与 Run 基础事件流。
- 补充 `AiSdkExecutor` 模板。
- 实现 `POST /runtime/runs` 与 `GET /runtime/runs/:runId/events`。
