...
- HubServer 和 Runtime 依赖 native/dynamic 包，例如 libsql、sharp、node-pty、pino/thread-stream worker 和外部 agent SDK bundled binaries。当前 HubServer 终端功能通过 `pty-session-host.cjs` 加载 `node-pty`，Runtime 日志 transport 通过 `thread-stream` 加载 worker，因此这些 helper、worker 和 native/dynamic 依赖必须随服务分发。
...
- 建立发行包 smoke，覆盖 Web、HubServer health、Runtime sidecar、Prisma/libsql、sharp、HubServer terminal PTY helper/node-pty、Runtime pino/thread-stream worker 和外部 agent SDK binary resolution。
