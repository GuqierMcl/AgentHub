  ...
  buildSystemPrompt(context: AgentExecutionContext): string {
    ...
    return [
      ...
      [
        ...
        "- When a delegated task may write known workspace files, pass run_task.lockPaths with exact workspace-relative file paths such as [\"src/App.tsx\"].",
        "- Do not parallelize tasks that may write the same file or tightly coupled API/interface files; use dependsOn to serialize them.",
        "- If the write target files are unknown, first delegate a read-only discovery task without lockPaths, then delegate write tasks with lockPaths after the file list is known.",
        "- If run_task fails with TASK_FILE_LOCK_CONFLICT, update the plan and resolve the conflict instead of ignoring it: wait, retry serially, assign a review/merge task, or explain the blocker.",
